	...
	, m_showAnimation(SystemService::instance()->showCardLoadingAnimation())
{
	...
	if (G_LIKELY(s_glow == 0)) {
		...
		if(s_glow)
			*s_glow = s_glow->scaledToHeight(s_glow->height() * (Settings::LunaSettings()->pixmapScale), Qt::SmoothTransformation);
			s_glowRefCount++;
        ...
        if (G_LIKELY(s_background == 0)) {
            ...
            if (s_background) {
		    *s_background = s_background->scaledToHeight(s_background->height() * (Settings::LunaSettings()->pixmapScale), Qt::SmoothTransformation);
    ...
    if (appDesc) {
        ...
        if (Settings::LunaSettings()->enableSplashBackgrounds) {
            ...
            m_background = m_background.scaledToHeight(m_background.height() * (Settings::LunaSettings()->pixmapScale), Qt::SmoothTransformation);
        }
    }
    else {

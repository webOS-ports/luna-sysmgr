...
static const int kAlertWindowWidth = 320 * Settings::LunaSettings()->uiScale;
...
void WindowServerLuna::drawBackground ( QPainter * painter, const QRectF & rect )
{
	...
	QRect imgBounds = QRect(-(int)m_currWallpaperImg->width()/2,
				...
				m_currWallpaperImg->width(),
				m_currWallpaperImg->height());
...
void WindowServerLuna::createMemoryAlertWindow()
{
	...
	int height = 160 * Settings::LunaSettings()->uiScale;
...
void WindowServerLuna::createDismissCardWindow()
{
	...
	int width = Settings::LunaSettings()->tabletUi ? kAlertWindowWidth : SystemUiController::instance()->currentUiWidth();
	int height = 170 * Settings::LunaSettings()->uiScale;

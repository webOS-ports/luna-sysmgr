	...
	, m_showBatteryText(showText)
{
	...
	m_imgWidth  = 17 * (Settings::LunaSettings()->uiScale / 4);
	m_imgHeight = 20 * (Settings::LunaSettings()->uiScale / 4);
	...
	m_errorPixmap = m_errorPixmap.scaledToHeight(m_errorPixmap.height() * (Settings::LunaSettings()->uiScale / 4), Qt::SmoothTransformation);
	...
	if(Settings::LunaSettings()->tabletUi && m_showBatteryText) {
		...
		m_font = new QFont(fontName, 14 * Settings::LunaSettings()->uiScale);
		m_font->setPixelSize(14 * Settings::LunaSettings()->uiScale);
...
void StatusBarBattery::init()
{
	...
	for (i = 0; i < kNumBatteryStates; i++) {
		...
		if (i == (kNumBatteryStates - 1)) {
			...
			m_pixmap[i] = m_pixmap[i].scaledToHeight(m_pixmap[i].height() * (Settings::LunaSettings()->uiScale / 4), Qt::SmoothTransformation);
		}

		m_chargingPixmap[i] = QPixmap(chargingPath.c_str());
		m_chargingPixmap[i] = m_chargingPixmap[i].scaledToHeight(m_chargingPixmap[i].height() * (Settings::LunaSettings()->uiScale / 4), Qt::SmoothTransformation);
	}

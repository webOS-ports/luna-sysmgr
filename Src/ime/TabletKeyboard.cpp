...
const int cPopupFontSize = 22 * Settings::LunaSettings()->textScale * 0.75;
...
const int cPopupTopToKey = 10 * Settings::LunaSettings()->textScale * 0.75;
...
const int cElipsisFontSize = 14 * Settings::LunaSettings()->textScale * 0.75;
...
public:
	...
	{
		if (maxWidth >= 1024 * Settings::LunaSettings()->textScale || maxHeight >= 1024 * Settings::LunaSettings()->textScale)
	...
	m_glyphCache(440, 800)
{
	...
	m_presetHeight[cKey_Resize_Tiny - cKey_Resize_First] = 243 * Settings::LunaSettings()->textScale * 0.75;
	m_presetHeight[cKey_Resize_Small - cKey_Resize_First] = ((340 + 243) / 2) * Settings::LunaSettings()->textScale * 0.75;
	m_presetHeight[cKey_Resize_Default - cKey_Resize_First] = m_background.height() * Settings::LunaSettings()->textScale * 0.75;
	m_presetHeight[cKey_Resize_Large - cKey_Resize_First] = 393 * Settings::LunaSettings()->textScale * 0.75;
...
//	bool		capitalize = key >= Qt::Key_A && key <= Qt::Key_Z;
	...
	if (text.size() > 0)
	{
		...
		int fontSize = 24 * Settings::LunaSettings()->textScale * 0.75;
		if (!twoVertical && !twoHorizontal && !UKeyIsFunctionKey(key))
			fontSize = 26 * Settings::LunaSettings()->textScale * 0.75;
		int centerOffset = 1;
		...
		if (text.size() > 1)
		{
			...
			fontSize = qMin<int>(fontSize, 22 * Settings::LunaSettings()->textScale * 0.75);
			sFont.setPixelSize(fontSize);
			int gap;
